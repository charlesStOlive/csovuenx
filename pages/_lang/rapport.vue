<template>
  <div>
    ...
    <v-card class="white_tr ma-4" v-if="ready">
      <v-toolbar color="primary" dark tabs>
        <v-toolbar-title>Votre rapport</v-toolbar-title>
        <MHelp title="Explications">
          J’ai simulé la création de données pour un CRM que j'ai développé dans mon Hub de données.
          Il y a :
          <ul>
            <li>25 000 ventes</li>
            <li>50 commerciaux</li>
            <li>100 clients</li>
            <li>5 gammes</li>
          </ul>Par défaut vous avez le Rapport de la semaine prescédente sur la première région. Vous pouvez à tout moment changer de semaine ou de région.
          <p>J'ai tout à fait conscience que la pertinence des mesures reportée dans cet exemple est limitée, mais c'est uniquement un exemple...</p>
        </MHelp>
        <v-spacer></v-spacer>
        <v-combobox
          placeholder="Modifier Région"
          v-model="selectedRegions"
          :items="rapport.regions"
          item-text="name"
          return-object
        />
        <v-combobox
          placeholder="Modifier Semaine"
          v-model="selectedMonths"
          :items="rapport.old_report"
          item-text="name"
          return-object
        />
        <v-btn
          @click="changeRapport()"
          :loading="loading"
          :disabled="show_btn_changr"
          class="grey"
        >Modifier</v-btn>
        <v-btn @click="createUrlPdf()" class="grey">Télecharger</v-btn>
      </v-toolbar>
      <div class="pa-3">
        ...
        Semaine analysé du
        <b>{{rapport.global.weekStart}} au {{ rapport.global.weekEnd }}</b> |
        N° de semaine :
        <b>{{ rapport.global.weekNum }}</b> |
        Pour la région :
        <span class="primary--text">{{ rapport.region.name }}</span>
      </div>
      <v-layout wrap row>
          ...
          <v-layout wrap row>
            <v-flex md6>
              <ul>
                ...
                <ul>
                  <li
                    v-for="commercial in rapport.region.best_week_sales_sum"
                    :key="commercial.id"
                  >{{ commercial.name }} : {{ commercial.week_sales_sum }} €</li>
                </ul>
                <li>Ventes année N : {{ rapport.region.compil_total_sales }} €</li>
                <li>Top 3 commerciaux année N (CA)</li>
                <ul>
                  <li
                    v-for="commercial in rapport.region.best_compil_sales_sum"
                    :key="commercial.id"
                  >{{ commercial.name }} : {{ commercial.compil_sales_sum }} €</li>
                </ul>
              </ul>
            </v-flex>
            <v-flex md6>
          ...
          <v-layout wrap row>
            <v-flex md6>
              <ul>
                ...
                <ul>
                  <li
                    v-for="commercial in rapport.all.best_week_sales_sum"
                    :key="commercial.id"
                  >{{ commercial.name }} : {{ commercial.week_sales_sum }} €</li>
                </ul>
                ...
                <ul>
                  <li
                    v-for="commercial in rapport.all.best_compil_sales_sum"
                    :key="commercial.id"
                  >{{ commercial.name }} : {{ commercial.compil_sales_sum }} €</li>
                </ul>
              </ul>
        ...
        <v-flex md6 class="pa-2">
          ...
          <v-layout wrap row>
            <v-flex md6>
              <ul>
                <li
                  v-for="(item, index) in rapport.region.repartition_gammes"
                  :key="index"
                >{{ index }} : {{ item }} €</li>
              </ul>
            </v-flex>
            <v-flex md6>
              <DoughnutChartModel
                class="pt-4"
                height="300px"
                :data="rapport.region.repartition_gammes"
              />
            </v-flex>
          </v-layout>
        </v-flex>
        ...
        <v-flex md6 class="pa-2">
          ...
          <v-layout wrap row>
            <v-flex md6>
              <ul>
                <li
                  v-for="(item, index) in rapport.all.repartition_gammes"
                  :key="index"
                >{{ index }} : {{ item }} €</li>
              </ul>
            </v-flex>
            <v-flex md6>
              <DoughnutChartModel
                class="pt-4"
                height="300px"
                :data="rapport.all.repartition_gammes"
              />
            </v-flex>
          </v-layout>
        </v-flex>
...
import MHelp from "@/components/Modal/MHelp";
