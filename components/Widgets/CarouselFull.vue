  ...
  <v-layout row wrap >
    <v-flex xs12>
        ...
        <div v-if="slideData != null" class="pa-2 text-xs-center">
          ...
          <v-btn outline color="primary" :to="$i18n.path('projects/'+slideData.slug)"><v-icon >fa-eye</v-icon>&nbsp;{{ $t('widget.carousel.show_project') }}<span v-if="slideData.client"> : {{ slideData.client.name }}</span></v-btn>
        </div>
    </v-flex>
  </v-layout>
